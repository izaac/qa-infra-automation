...
run_playbook() {
	$RUNTIME run --rm -it \
		...
		-e AZURE_AKS_SUBSCRIPTION_ID="${AZURE_AKS_SUBSCRIPTION_ID:-}" \
		-e GKE_SERVICE_ACCOUNT="${GKE_SERVICE_ACCOUNT:-}" \
		"$IMAGE_NAME" \
		"$@"
}
...
stream_cypress() {
	...
	if ! $RUNTIME image inspect dashboard-test:latest >/dev/null 2>&1; then
		echo "ERROR: dashboard-test:latest image not found — setup may have failed" >&2
		exit 1
	fi

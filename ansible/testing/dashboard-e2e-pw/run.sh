#!/bin/sh
# Wrapper to run the dashboard-e2e-pw playbook inside a container.
...
IMAGE_NAME="dashboard-e2e-pw-runner"
...
# --- Stream Playwright with live output ---
stream_playwright() {
	echo ""
	echo "[run] Streaming Playwright tests..."
	echo ""

	if ! $RUNTIME image inspect dashboard-pw-test:0 >/dev/null 2>&1; then
		echo "ERROR: dashboard-pw-test:0 image not found — setup may have failed" >&2
		exit 1
	fi
	...
	_host="$(grep '^rancher_host:' "$VARS_FILE" 2>/dev/null | head -1 | sed "s/^rancher_host:[[:space:]]*//" | tr -d "\"'" || echo "dashboard-e2e-pw")"
...
# stream mode runs the test phase directly via `stream_playwright` instead of
# inside ansible — but it still needs the rest of the pipeline to run through
# ansible. Make sure `setup` is always part of TAGS so the Docker image gets
# built; otherwise stream_playwright fails with "image not found".
if [ -n "$STREAM" ]; then
	case "${TAGS}" in
	"")
		TAGS="setup,test"
...
if [ ! -f "$VARS_FILE" ]; then
	...
	echo "    rancher_host, rancher_password, rancher_image_tag, pw_grep_tags" >&2
	echo "" >&2
	exit 1
fi

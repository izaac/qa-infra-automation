...
Run Dashboard **Playwright** E2E tests with just Docker and a config file.
...
cd qa-infra-automation/ansible/testing/dashboard-e2e-pw
```
...
pw_grep_tags: "@generic"

dashboard_pw_repo: "rancher/dashboard-e2e-pw"
dashboard_pw_branch: "main"
```
...
>
> `stream` gives you **real-time Playwright output with colors**.
...
tweak `pw_grep_tags` or dashboard-e2e-pw code.

| What changed | Command |
| --- | --- |
| Only `pw_grep_tags` in `vars.yaml` | `./run.sh stream test` |
| Other vars that affect `.env` | `./run.sh stream setup test` |
| dashboard-e2e-pw source code | `./run.sh stream` |
...
# 2. Change pw_grep_tags in vars.yaml, re-run (seconds to start)
./run.sh stream test

# 3. Fix a test, re-clone repo + rebuild, test again
./run.sh stream
...
```bash
# JUnit XML
ls dashboard-e2e-pw/e2e/jenkins/reports/junit/results.xml

# HTML report with screenshots/traces
ls dashboard-e2e-pw/playwright-report/
```
...
./run.sh results
# or
open dashboard-e2e-pw/playwright-report/index.html 2>/dev/null || \
  xdg-open dashboard-e2e-pw/playwright-report/index.html
```
...
./run.sh stream                        # setup + test, live Playwright output
...
./run.sh results                       # open the HTML report
./run.sh clean                         # remove dashboard-e2e-pw/, outputs/, .env
...
## Playwright grep examples

Playwright's `--grep` accepts a JavaScript regular expression matched against
each test's full title. Use this instead of Cypress's `@tag` combinators.

| Grep | What it matches |
| --- | --- |
| `@generic` | Tests titled with `@generic` (login, home, about — no clusters) |
| `@adminUser` | Admin workflows — needs default Rancher setup |
| `@standardUser` | Standard-user permission tests |
| `@manager.*@adminUser` | Tests carrying both `@manager` and `@adminUser` |
| `@(manager\|monitoring)` | Either tag |

See [Playwright tag annotations](https://playwright.dev/docs/test-annotations#tag-tests).
...
ansible-playbook dashboard-e2e-pw-playbook.yml --tags setup

# Re-run tests only
ansible-playbook dashboard-e2e-pw-playbook.yml --tags test
```

For real-time Playwright output with colors, run Docker manually
after setup:
...
- **Variable reference:** See `vars.yaml.example` for all knobs.
